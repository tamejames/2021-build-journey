...
# Model M: Finding M (magenta) label based on RGB values as features
...
# 0. ABOUT THIS MODEL
# This model will take RGB values and predict a 'M percent by paint vol' value.
# This is the proportion of acrylic magenta paint (by volume) required to recreate the RGB colour based on physical experiments. The data for these experiments are stored
...
# Set label for 'M percent by paint vol'
labels = image_data.loc[:,'M percent by paint vol']
...
filename = 'm_guess_model_linreg.sav'
pickle.dump(reg,open(filename,'wb'))
loaded_model = pickle.load(open('m_guess_model_linreg.sav', 'rb'))
