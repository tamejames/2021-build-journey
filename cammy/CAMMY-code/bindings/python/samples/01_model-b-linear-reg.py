...
# Model B: Finding B (black) label based on RGB values as features
...
# 0. ABOUT THIS MODEL
# This model will take RGB values and predict a 'B percent by paint vol' value.
# This is the proportion of acrylic black paint (by volume) required to recreate the RGB colour based on physical experiments. The data for these experiments are stored
...
# Set label for 'B percent by paint vol'
labels = image_data.loc[:,'B percent by paint vol']
...
filename = 'b_guess_model_linreg.sav'
pickle.dump(reg,open(filename,'wb'))
loaded_model = pickle.load(open('b_guess_model_linreg.sav', 'rb'))
